                    ...
                    try {
                        ...
                        switch (data.type) {
                            ...
                            case "token_info":
                                ...
                                const metadataEl =
                                    currentMessageEl.querySelector(".message-metadata");
                                if (metadataEl) {
                            ...
                            case "tool_content":
                                ...
                                if (currentToolCall) {
                                    const contentEl =
                                        currentToolCall.querySelector(".tool-content");
                            ...
                            case "tool_result":
                                ...
                                if (currentToolCall) {
                                    ...
                                    if (
                                        content.includes("Successfully converted") &&
                                        ...
                                        const filePaths = lines
                                            .slice(1)
                                            .filter((line) => line.trim().endsWith(".xlsx"));

                                        // Create download links for Excel files
                                        if (filePaths.length > 0) {
                                            const baseContent = lines[0];
                                            const fileLinks = filePaths
                                                .map((path) => {
                                                    const fileName = path
                                                        .split("/")
                                                        .pop()
                                                        .split("\\")
                                                        .pop(); // Handle both Unix and Windows paths
                                                    // Get file size if available
                                                    const fileSize = "(Haga clic para descargar)";

                                                    return `<a href="/download?path=${encodeURIComponent(
                                                        path
                                                    )}" 
                                            ...
                                            setTimeout(() => {
                                                ...
                                                console.log(
                                                    "Files generated event dispatched:",
                                                    filePaths
                                                );
                                            }, 500);
                                        }
        ...
        try {
            ...
            const response = await fetch("/api/files")
                ...
                .catch(() => {
                    ...
                    return fetch("/api/search_files").then((res) => (res.ok ? res.json() : []));
                });
